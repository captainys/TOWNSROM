...
void InitENVSEG(unsigned int ENVSEG,unsigned int len,const char path[])
{
	...
	int useFileNameBuffer0=0;
	char far *fileNameBuffer0=DOSGETSYSVARS();
	FP_OFF(fileNameBuffer0)=DOS_SYSVARS_FILENAME_BUFFER0;
	...
	// About COMSPEC.  This program may be saved as COMMAND.COM or YAMAND.COM.
	// I want to set COMSPEC as the file name as it started.
	// However, the top-level command processor does not have an ENVSEG where
	// the process file name is written, which creates a chicken-and-egg problem.
	// The only place I can find the file name seems to be file-name buffer 1
	// of DOS, which is at DOSSEG:0360H
	for(j=0; 0!=COMSPEC[j]; ++j)
	{
		ptr[i++]=COMSPEC[j];
	}

	for(j=0; 0!=fileNameBuffer0[j]; ++j)
	{
		if(0==_fstrcmp(fileNameBuffer0+j,"COMMAND.COM") ||
		   0==_fstrcmp(fileNameBuffer0+j,"YAMAND.COM"))
		{
			useFileNameBuffer0=1;
			break;
		}
	}

	if(0==useFileNameBuffer0)
	{
		for(j=0; 0!=path[j]; ++j)
		{
			ptr[i++]=path[j];
